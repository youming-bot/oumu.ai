...
export async function generateWaveform(
  ...
  try {
    ...
    const smoothedPeaks = smoothing ? smoothPeaks(peaks, smoothingWindow) : peaks;
...
export function normalizePeaks(peaks: number[], maxHeight: number = 1): number[] {
...
export function compressPeaks(peaks: number[], targetLength: number): number[] {
