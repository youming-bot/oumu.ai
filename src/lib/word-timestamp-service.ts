import { WordTimestamp, Segment } from '@/types/database';
...
export class WordTimestampService {
  ...
  static getWordContext(
    ...
    const previous = wordTimestamps.slice(
      Math.max(0, currentIndex - contextWords),
      currentIndex
    );
   ...
   */
  static findPauses(wordTimestamps: WordTimestamp[], pauseThreshold: number = 0.3): Array<{
    start: number;
    end: number;
    duration: number;
  }> {
