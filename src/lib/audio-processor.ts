...
export async function sliceAudio(
  ...
  try {
    ...
    while (currentStart < endTime) {
      ...
      let chunkBlob: Blob;
      try {
        console.log(`🧪 TEST: Inside try block for chunk ${chunkIndex}`);
        console.log(`🎛️ About to call extractAudioSegment for chunk ${chunkIndex}...`);

        // 简单测试：先创建一个空的blob来测试流程
        console.log(`🧪 TEST: Creating simple test blob for chunk ${chunkIndex}`);
        chunkBlob = new Blob(['test'], { type: 'audio/wav' });
        console.log(`✅ Test blob created for chunk ${chunkIndex}, size:`, chunkBlob.size);

        // 暂时注释掉实际的extractAudioSegment调用
        ...
        //   audioContext,
        // );
        console.log(`✅ Chunk ${chunkIndex} extracted, size:`, chunkBlob.size);
      } catch (extractError) {
        ...
        console.error('Error details:', {
          ...
          stack: extractError instanceof Error ? extractError.stack : 'No stack trace',
        });
        throw extractError;
      }
    ...
    console.log('🎉 Chunk creation loop completed, total chunks:', chunks.length);
...
export async function processAudioFile(
  ...
  console.log('🔄 processAudioFile called with:', {
    fileBlobOrId: typeof fileBlobOrId === 'number' ? `fileId: ${fileBlobOrId}` : 'direct blob',
    chunkSeconds,
    overlap,
  });
