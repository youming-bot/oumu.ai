      ...
      try {
        ...
        const uploadPromises = selectedFiles.map(async (file, index) => {
          try {
            ...
            console.log(`🎵 Starting transcription for file: ${file.name} (ID: ${fileId})`);
            TranscriptionService.transcribeAudio(fileId, {
              language: 'ja',
              onProgress: (progress) => {
                console.log(`📊 Transcription progress for ${file.name}:`, progress);
              },
            })
              .then((transcriptId) => {
                console.log(
                  `✅ Transcription completed for ${file.name}. Transcript ID:`,
                  transcriptId
                );
                toast.success(`Transcription completed for ${file.name}`);
              })
              .catch((transcriptionError) => {
                console.error(`❌ Transcription failed for ${file.name}:`, transcriptionError);
