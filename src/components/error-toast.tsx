        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mb-1 flex items-start justify-between">
              <h4 className="truncate pr-2 font-medium text-gray-900 text-sm">{config.title}</h4>
              {config.dismissible && (
                <Button
                  variant="ghost"
                  size="sm"
            ...
            {/* 消息 */}
            {config.message && <p className="mb-2 text-gray-600 text-sm">{config.message}</p>}

            {/* 描述 */}
            {config.description && (
              <p className="mb-3 text-gray-500 text-xs">{config.description}</p>
            )}

            {/* 详细信息 */}
            {config.showDetails && config.details && (
              <div className="mb-3 rounded border border-gray-200 bg-white p-2">
                <div className="text-gray-600 text-xs">{config.details}</div>
              </div>
            )}
        ...
        details: (
          <div className="space-y-1">
            <div className="flex items-center justify-between">
              <span className="text-gray-500 text-xs">错误代码:</span>
            ...
            {error.statusCode && (
              <div className="flex items-center justify-between">
                <span className="text-gray-500 text-xs">状态码:</span>
                <Badge variant="outline" className="text-xs">
                  {error.statusCode}
                </Badge>
              </div>
        ...
        details: (
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="text-gray-500">总错误数:</div>
              <div className="text-right">{stats.totalErrors}</div>
              <div className="text-gray-500">错误频率:</div>
              <div className="text-right">{stats.errorFrequency}/小时</div>
            </div>
            {stats.lastErrorTime && (
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-500">最后错误时间:</span>
                <span>{new Date(stats.lastErrorTime).toLocaleTimeString()}</span>
              </div>
            )}
            {Object.keys(stats.errorsByCode).length > 0 && (
              <div>
                <div className="mb-1 text-gray-500 text-xs">常见错误:</div>
                <div className="space-y-1">
