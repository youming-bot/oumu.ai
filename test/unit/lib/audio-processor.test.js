...
describe('Audio Processor', () => {
  ...
  describe('getAudioDuration', () => {
    ...
    test('should handle audio loading errors', async () => {
      ...
      // Act & Assert
      await expect(AudioProcessor.getAudioDuration(mockBlob)).rejects.toThrow('Failed to load audio metadata');
    });
  });
  ...
  describe('processAudioFile', () => {
    test('should process audio file and return chunks', async () => {
      ...
      expect(AudioProcessor.sliceAudio).toHaveBeenCalledWith(
        mockBlob, 0, 60, 45, 0.2
      );
    });
  ...
  describe('sliceAudio', () => {
    test('should slice audio into chunks with overlap', async () => {
      ...
      jest.spyOn(AudioProcessor, 'extractAudioSegment').mockImplementation(async (buffer, start, end, ctx) => {
      ...
      expect(chunks[1]).toEqual(expect.objectContaining({
        startTime: 28, // 30 - 2 overlap
        endTime: 58,
        duration: 30,
        index: 1
      }));
      expect(chunks[2]).toEqual(expect.objectContaining({
        startTime: 56, // 58 - 2 overlap
        endTime: 86,
        duration: 30,
        index: 2
      }));
      expect(chunks[3]).toEqual(expect.objectContaining({
        startTime: 84, // 86 - 2 overlap
        endTime: 100, // Clamped to end time
        duration: 16,
        index: 3
      }));
    ...
    test('should handle audio decoding errors', async () => {
      ...
      // Act & Assert
      await expect(AudioProcessor.sliceAudio(mockBlob, 0, 100, 30, 2))
        .rejects.toThrow('Audio processing error: Decoding failed');
      expect(mockAudioContext.close).toHaveBeenCalled();
    });
  });
  ...
  describe('mergeAudioChunks', () => {
    test('should merge audio chunks into single blob', async () => {
      ...
      const mockAudioContext = {
        decodeAudioData: jest.fn().mockImplementation(async (buffer) => ({
          getChannelData: jest.fn().mockReturnValue(new Float32Array(1323000).fill(0.5)) // 30s at 44100Hz
        })),
        createBuffer: jest.fn().mockReturnValue({
          getChannelData: jest.fn().mockReturnValue(new Float32Array(2646000)) // 60s at 44100Hz
        }),
        close: jest.fn()
      };
    ...
    test('should handle audio processing errors during merge', async () => {
      ...
      // Act & Assert
      await expect(AudioProcessor.mergeAudioChunks(mockChunks)).rejects.toThrow('Decoding failed');
      expect(mockAudioContext.close).toHaveBeenCalled();
    });
  });
