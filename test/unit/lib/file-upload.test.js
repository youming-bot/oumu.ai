...
describe('File Upload Utilities', () => {
  ...
  describe('uploadFile', () => {
    test('should upload valid audio file', async () => {
      ...
      expect(DBUtils.addFile).toHaveBeenCalledWith(expect.objectContaining({
        name: 'test.mp3',
        size: 5 * 1024 * 1024,
        type: 'audio/mp3'
      }));
    ...
    test('should throw error for invalid file type', async () => {
      ...
      // Act & Assert
      await expect(FileUploadUtils.uploadFile(invalidFile)).rejects.toThrow('Only audio files are supported');
    ...
    test('should throw error for oversized file', async () => {
      ...
      // Act & Assert
      await expect(FileUploadUtils.uploadFile(largeFile)).rejects.toThrow('File size must be less than 100MB');
      expect(DBUtils.addFile).not.toHaveBeenCalled();
    });
    ...
    test('should handle database errors during upload', async () => {
      ...
      // Act & Assert
      await expect(FileUploadUtils.uploadFile(mockFile)).rejects.toThrow('Upload failed: Database error');
    });
  });
  ...
  describe('updateFileMetadata', () => {
    ...
    test('should rethrow database errors', async () => {
      ...
      // Act & Assert
      await expect(FileUploadUtils.updateFileMetadata(123, { name: 'test' })).rejects.toThrow('Database error');
    });
  });
