...
import { FileRow, TranscriptRow } from '../../../src/types/database';
...
jest.mock('@/components/ui/button', () => {
  return function MockButton({ children, onClick, disabled, variant, size, className }) {
    return (
      <button
        onClick={onClick}
        disabled={disabled}
...
jest.mock('@/components/ui/badge', () => {
  return function MockBadge({ children, variant, className }) {
...
jest.mock('@/components/ui/skeleton', () => {
  return function MockSkeleton({ className }) {
    return <div className={className} data-testid="skeleton">Loading...</div>;
  };
});
...
describe('FileList Component', () => {
  ...
  const createMockFile = (overrides = {}): FileRow => ({
    id: 1,
    name: 'test-audio.mp3',
  ...
  describe('Loading State', () => {
    it('should show skeleton loading when isLoading is true', () => {
      ...
      expect(screen.getAllByTestId('skeleton')).toHaveLength(15); // 3 rows × 5 skeletons per row
    });
  ...
  describe('Empty State', () => {
    it('should show empty state when no files and not loading', () => {
      ...
      expect(screen.getByText('Upload audio files to get started with shadowing practice.')).toBeInTheDocument();
  ...
  describe('File Display', () => {
    ...
    it('should format different file sizes correctly', () => {
      const files = [
        ...
        createMockFile({ id: 4, name: 'huge.mp3', size: 1024 * 1024 * 1024 * 2.5 }), // 2.5 GB
      ];
  ...
  describe('Badge Variants', () => {
    it('should use correct badge variants for different statuses', () => {
      ...
      const statusBadges = screen.getAllByText(/Completed|Processing|Failed|Pending/);
